<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-else @click="selAddress">
        请选择配送地址
      </div>
...
<script>
import { checkOrder, getAddressList } from '@/api/order'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  methods: {
    async getOrderList () {
      if (this.mode === 'cart') {
        ...
        console.log(order)
        this.longAddress = order.address.region.province + order.address.region.city + order.address.region.region + order.address.detail
        this.order = order
